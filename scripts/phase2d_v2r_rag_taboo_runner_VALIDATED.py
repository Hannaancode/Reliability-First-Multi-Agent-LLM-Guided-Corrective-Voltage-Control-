...
# ============================================================
# EXACT TRUSTED v2 IMPORT BLOCK - ADDED BY PATCH
# ============================================================
...
# Remove stale cached Phase 2 modules that may pollute scenario application
...
# Use v2 candidates, but add -50 Mvar because the old RAG paper used 50 Mvar-style injections.
...
def normalize_bus_id(net, b):
    ...
    try:
        ...
        # Some metadata may use 1-based bus numbering.
        if (bi - 1) in set(net.bus.index):
            return bi - 1
    except Exception:
        pass

    # Try matching net.bus.name.
    try:
...
def update_summary(v2b_csv=None):
    ...
    # ============================================================
    # PATCH: force numeric metric columns before summary statistics
    # Prevents pandas object/string concatenation during mean().
    # ============================================================
    _numeric_cols = [
...
def main():
    ...
    # Fresh run: remove old outputs for this diagnostic.
