...
#
# IMPORTANT:
# - This script does NOT change verifier/fallback/physics logic.
# - The LLM branch uses Reflection-v2 prompt/memory only.
# - The final selector chooses the better verified trajectory by final UV/deficit.
# - Run this AFTER your normal Phase 2G restore/setup cells are complete.
# ============================================================
...
# ------------------------------------------------------------
# Make /content imports work in Colab
# ------------------------------------------------------------
sys.path.insert(0, "/content")

import phase2g_llm_guided_rescue_runner as runner

# Force the intended experiment budget. This is not a new algorithmic change;
# it enforces the paper's fixed T=25 setting.
...
# No-LLM fallback-first results extracted from your pasted log.
...
""".strip()
    ...
    # Simple JSON schema. This is easier for a 0.5B model than nested JSON.
    user_text = f"""
...
def normalize_compact_or_nested_action(parsed):
    ...
    # The rescue normalizer already supports compact and nested forms in your patched runner.
...
def run_one_scenario_two_best_v2(scenario, base_net, zones, v2, model, tokenizer, mode):
    ...
    for it in range(runner.MAX_ITERATIONS):
        ...
        # Ask all zone agents. Same design as existing runner.
        ...
        # Original router source-priority design unchanged.
        priority = {
            "llm_direct": 3,
...
def run_one_scenario_no_llm_fallback_first(scenario, base_net, zones, v2):
    ...
    for it in range(runner.MAX_ITERATIONS):
        ...
        # Same fallback generator as the original LLM-guided runner, but with no LLM actions.
...
# ============================================================
# Execute when run via exec(open(...).read())
# ============================================================
